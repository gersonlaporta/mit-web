<?php
/**
 * API CIFO /api/public/biblioteca/temas
 *
...
switch ($_SERVER ['REQUEST_METHOD']) {
  case 'GET' :
    $objTema = new tema ( 'json' );
    if (isset ( $arrURI [0] )) {    // /api/public/biblioteca/temas/?
    	if($arrURI [0]=='seccion'){ // /api/public/biblioteca/temas/seccion/{id_secciones}
        echo '{
          "status"  : true,
		  "msg"		: "",
          "records" : '.$objTema->getAll('','', $arrURI [1]).'
        }';
    	}else{              // /api/public/biblioteca/temas/{id}
        echo '{
          "status"  : true,
		   "msg"	: "",
          "record"  :'. $objTema->get ($arrURI [0] ).'
      }';
      }
      
    } else {              // /api/public/biblioteca/temas/
      echo '{
          "status"  : true,
		  "msg"		: "",
          "records" : '.$objTema->getAll().'
        }';
    }
    break;
  case 'OPTIONS' :
