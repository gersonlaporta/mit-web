<?php
/**
 * API CIFO /api/public/biblioteca/editoriales
 *
...
switch ($_SERVER ['REQUEST_METHOD']) {
  case 'GET' :
    $objEditorial = new editorial ( 'json' );
    if (isset ( $arrURI [0] )) {    // /api/public/biblioteca/editoriales/{id}
      
      echo '{ 
          "status"  : true,
		   "msg"		:"",
          "record"  :'. $objEditorial->get ( $arrURI [0] ).'
        }';
    } else {              // /api/public/biblioteca/editoriales
      
      echo '{
          "status"  : true,
 		   "msg"		:"",
          "records" : '.$objEditorial->getAll().'
        }';
    }
    break;
  case 'OPTIONS' :
