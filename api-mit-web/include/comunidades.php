 ...
 * Class comunidad
 *
 * Class de comunidad con los metodos de acceso a la tabla de tbl_comunidades
 *
...
class comunidad extends  db {
   /**
     * Constructor de la class comunidad
    ...
    */
    public function get($id){
        ...
        switch($this->returnType){
           ...
           case 'xml':
                	...
                	$xml->encoding ='UTF-8';
                    $rootTag=$xml->createElement('comunidad');
    ...
    /**
     * Obtener comunidades , permite pasar parametros de paginacion
    ...
    public function getAll($start='',$page='' ){
        ...
        switch($this->returnType){
            ...
            case 'xml':
            	$dom = new DOMImplementation();
            	$dtd=  'comunidades [';
            	$dtd.= '<!ELEMENT comunidades (comunidad*)>  ';
            	$dtd.= '<!ELEMENT comunidad (';
            	foreach($fields as $field){
            		$dtd.= ($first_field)? '':',';
            		$first_field=false;
            	...
            	$xml=$dom->createDocument(null, null, $doctype);
            	$xml->encoding ='UTF-8';
                $rootTag=$xml->createElement('comunidades');
                for($k=0; $k<count($arrRows);$k++){
                    $itemTag=$xml->createElement('comunidad');
