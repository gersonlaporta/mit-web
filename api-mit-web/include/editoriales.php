 ...
 * Class editorial
 *
 * Class con los metodos de acceso a la tabla tbl_editoriales
 *
 * @author Alumne
...
class editorial extends  db {
	/**
	 * Constructor de la class editorial
	...
	function __construct($type='') {
		...
		$this->tabla='tbl_editoriales';
	}
    /**
     * get: Obtiene un registro de la tabla tbl_editoriales
     *
	...
	public function get($id=0){
		...
		switch($this->returnType){
			...
			case 'xml':
				...
				$xml->encoding ='UTF-8';
				$rootTag=$xml->createElement('editorial');
	...
	/**
	 * Obtener editoriales , permite pasar parametros de paginacion
	...
	public function getAll ($start='',$page=''){
		...
		switch($this->returnType){
			...
			case 'xml':
				...
				$dtd=  'editoriales [';
				$dtd.= '<!ENTITY nbsp "&#160;">';
				$dtd.= '<!ELEMENT editoriales (editorial*)>  ';
				$dtd.= '<!ELEMENT editorial 	(';
				foreach($fields as $field){
					$dtd.= ($first_field)? '':',';
					$first_field=false;
				...
				$xml=$dom->createDocument(null, null, $doctype);
				$xml->encoding ='UTF-8';
				$rootTag=$xml->createElement('editoriales');
				for($k=0; $k<count($arrRows);$k++){
					$itemTag=$xml->createElement('editorial');
